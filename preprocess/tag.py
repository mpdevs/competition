...
def process_tag(industry, table_name):
    ...
    FROM {0} WHERE ItemAttrDesc AND ItemAttrDesc != '';""".format(table_name)

    update_sql = """UPDATE {0}
    SET TaggedItemAttr=%s, TaggedBrandName=%s, TaggedMaterial=%s
    ...
    if n > 0:
        ...
        for cid in tag_dicts['CID'].unique():
            ...
            # x[3] = AttrValue
            # x[4] = Flag
            # type(tag_preparation): dict(list(dict))
...
def process_tag(industry, table_name):
    ...
    ItemAttrDesc AS Attribute, concat_ws(' ',ShopName,ItemSubTitle,ItemName) AS ShopNameTitle
    FROM {0}
    WHERE ItemAttrDesc IS NOT NULL AND ItemAttrDesc != '';""".format(table_name)
...
if __name__ == '__main__':
    process_tag('mp_women_clothing', 'itemmonthlysales2016')
