...
def process_annual(industry, db_name, table_from, table_to, one_shop, version):
    ...
    insert_sql = """
            INSERT INTO """+table_to+"""(SourceItemID,TargetItemID,RelationType,Status,ShopId,Version)
            VALUES
            ('%s',%s,'%s','%s', %s, %s)    
        """
    ...
    for value in shops:
        ...
        for i, item in enumerate(tqdm(items)):
            ...
            for j in xrange(len(todo_id)):
                ...
                if judge > 0:
                    insert_item = (item_id, todo_id[j], judge, 1, value, version)
