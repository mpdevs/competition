...
def process_annual(industry, table_from, table_to, one_shop=None):
    ...
    if status == 'Normal':        
        ...
        all_data = pd.read_sql_query(Select_sql[status] + table_from + " WHERE TaggedItemAttr IS NOT NULL;", connect_industry)   
    

    #模型训练
    training_data = pd.read_sql_query("Select ItemID, TaggedItemAttr from TaggedItemAttr where taggedItemAttr is not NULL", connect_industry)
    ...
    for value in shops:
        ...
        if status == 'Normal' or status == 'Monthly':
            ...
            cursor_industry.execute(Select_sql[status] + 'item' + " WHERE ShopID=%d AND TaggedItemAttr IS NOT NULL AND TaggedItemAttr!='';"%value)
        ...
        for i, item in enumerate(tqdm(items)):
            ...
            if cid2name[category_id] not in name2model.keys(): continue
            ...
            if status == 'Normal':
                ...
                insert_items += [(item_id, todo_id[j], 1, 1, value, todo_daterange[j]) for j in xrange(len(y)) if y[j][0] > y[j][1]]
        ...
        if n > 0:
            ...
            for j in tqdm(xrange(int(ceil(float(n)/batch)))):
                cursor_industry.executemany(Insert_sql[status], insert_items[j*batch:min((j+1)*batch, n)])
